...
const StarRating = ({ initialRating = 0, maxStars = 5, onRatingChange, preset = "star", iconActive, iconInactive, size = 30, colorActive, colorInactive = "#888", gap = 6, disabled = false, testID, }) => {
    ...
    return (<View style={[styles.starContainer, { gap }]}>
      {Array.from({ length: maxStars }, (_, i) => {
            ...
            return (<TouchableOpacity key={idx} onPress={() => !disabled && onRatingChange && onRatingChange(idx)} disabled={disabled} style={styles.starButton} accessibilityRole="button" accessibilityLabel={`Avaliar ${idx} de ${maxStars}`} testID={testID ? `${testID}-${idx}` : undefined} activeOpacity={0.7}>
