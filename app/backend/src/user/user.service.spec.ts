...
describe('UserService', () => {
    ...
    it('search rejects short query', async () => {
        await expect(service.searchUsersByUsername(1, 'a')).rejects.toThrow(BadRequestException);
    });
    it('search rejects bad limit', async () => {
        await expect(service.searchUsersByUsername(1, 'ab', 0)).rejects.toThrow(BadRequestException);
        await expect(service.searchUsersByUsername(1, 'ab', 51)).rejects.toThrow(BadRequestException);
    });
